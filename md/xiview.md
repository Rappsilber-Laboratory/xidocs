...
Firstly, cross-links, matches, protein-protein interactions and residue pairs [can be saved as CSV files](./export/csv.html). Matches can be saved in Skyline's SSL format, and Protein Accession Lists can be saved as text files. The final option "Make Filtered Xi URL" offers the option to export a URL that includes the current filter state. Upon re-opening in a browser this URL will set the filter to the values in the URL as a convenient shortcut.
